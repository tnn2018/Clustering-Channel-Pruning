...
def conv3x3_new1(in_planes, out_planes, stride=1):
    "3x3 convolution with padding"
    return nn.Conv2d(in_planes, 12, kernel_size=3, stride=stride,
                     padding=1, bias=False)
    print('conv')
def conv3x3_new2(in_planes, out_planes, stride=1):
    "3x3 convolution with padding"
    return nn.Conv2d(12, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=False)
    print('conv')
...
class BasicBlock_new(nn.Module):
    ...
    def __init__(self, inplanes, planes, cfg, stride=1, downsample=None):
        ...
        self.conv1 = conv3x3_new1(inplanes, cfg, stride)
        self.bn1 = nn.BatchNorm2d(12)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3_new2(cfg, planes)
        self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
        self.stride = stride
    def forward(self, x):
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, dataset='cifar10', cfg=None):
        ...
        block = BasicBlock
        block_new = BasicBlock_new  # add new
        if cfg == None:
            cfg = [[16]*n, [32]*n, [64]*n]
...
#        self.layer2 = self._make_layer(block_new, 32, n, cfg=cfg[n:2*n], stride=2)
...
#        self.layer3 = self._make_layer(block_new, 64, n, cfg=cfg[2*n:3*n], stride=2)
