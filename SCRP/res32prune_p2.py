...
parser.add_argument('--depth', type=int, default=32,
                    help='depth of the resnet')                       # checkpoint.pth.tar
parser.add_argument('--model', default='./ResNet/resnet32-c10/SC28/model_best.pth.tar', type=str, metavar='PATH',
                    help='path to the model (default: none)')
parser.add_argument('--save', default='./ResNet/resnet32-c10/SC28/pruned_test', type=str, metavar='PATH',
...
for m in model.modules():
    if isinstance(m, nn.Conv2d): 
        ...
        if layer_id % 2 == 0:
            ...
            clustering = SC(n_clusters=num_keep,eigen_solver=None, random_state=None, n_init=10, gamma=1.0, affinity='rbf',
               n_neighbors=10, eigen_tol=0.0, assign_labels='kmeans',degree=3, coef0=1,kernel_params=None, n_jobs=None).fit(weight_copy)
...
print(model)
print_model_param_flops(model=model, input_res=32, multiply_adds=True)
